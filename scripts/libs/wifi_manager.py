# Author: Igor Ferreira; License: MIT; Origin: https://github.com/ferreira-igor/micropython-wifi_manager
# Updated by: Alexandre Calil
...
class WifiManager:
    """Manage the WiFi connection through a web interface.

    When the device is turned on, connect to the WiFi network defined in the code,
    then access the web interface from 192.168.4.1 and fill in the SSID and password of the desired network.

    Attributes:
        wlan_sta: The WiFi interface used to connect to the network.
        wlan_ap: The WiFi interface used to create the access point.
        ap_authmode: Authentication mode of the access point. WPA2_PSK if a password is defined, OPEN otherwise.
        ap_ssid: SSID of the access point.
        ap_password: Password of the access point. if ap_authmode is WPA2_PSK.
        reboot_after: If device should reboot after configuration. Useful if you're having problems with web server applications after WiFi configuration.
        sta_profiles: The file were the credentials will be stored. There is no encryption, it's just a plain text archive. Be aware of this security problem!
    """
    ...
    sta_profiles = "wifi.dat"
    wlan_sta = network.WLAN(network.STA_IF)
    wlan_ap = network.WLAN(network.AP_IF)

    def __init__(self, ssid="Wifi Manager", reboot_after=False):
        """Inits WifiManager with Open AP

        Args:
            ssid: SSID of the access point.
            reboot_after: If device should reboot after configuration. Useful if you're having problems with web server applications after WiFi configuration.
        """
        self.ap_authmode = self.AUTH_MODES["OPEN"]
    ...
    def __init__(self, ssid="Wifi Manager", password="123456789", reboot_after=False):
        """Inits WifiManager with WPA2_PSK AP

        Args:
            ssid: SSID of the access point.
            password: Password of the access point. WPA2_PSK.
            reboot_after: If device should reboot after configuration. Useful if you're having problems with web server applications after WiFi configuration.
        """
        self.ap_authmode = self.AUTH_MODES["WPA2_PSK"]
        self.reboot_after = reboot_after
    ...
    def __WebServer(self):
        ...
        while True:
            ...
            try:
                ...
                if self.request:
                    url = (
                        ure.search("(?:GET|POST) /(.*?)(?:\\?.*?)? HTTP", self.request)
                        .group(1)
                        .decode("utf-8")
                        .rstrip("/")
                    )
    ...
    def __HandleConfigure(self):
        ...
        if match:
            ...
            if len(ssid) == 0:
                ...
                self.__SendResponse(
                    """<p>Successfully connected to</p><h1>{0}</h1><p>IP address: {1}</p>""".format(
